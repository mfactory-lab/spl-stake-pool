...
async function findEphemeralStakeProgramAddress(programId, stakePoolAddress, seed) {
    const [publicKey] = await web3_js.PublicKey.findProgramAddress([EPHEMERAL_STAKE_SEED_PREFIX, stakePoolAddress.toBuffer(), seed.toBuffer('le', 8)], programId);
    return publicKey;
}
...
async function getMetadataPDA(mint) {
    const [publicKey] = await web3_js.PublicKey.findProgramAddress([buffer.Buffer.from('metadata'), METADATA_PROGRAM_ID.toBuffer(), mint.toBuffer()], METADATA_PROGRAM_ID);
    return publicKey;
}
...
const MOVE_STAKE_LAYOUT = BufferLayout__namespace.struct([
    ...
    BufferLayout__namespace.nu64('lamports'),
    BufferLayout__namespace.nu64('transientStakeSeed'),
]);
const TOKEN_METADATA_LAYOUT = BufferLayout__namespace.struct([
    BufferLayout__namespace.u8('instruction'),
    BufferLayout__namespace.blob(METADATA_MAX_NAME_LENGTH, 'name'),
...
const STAKE_POOL_INSTRUCTION_LAYOUTS = Object.freeze({
    ...
    WithdrawStake: {
        ...
        layout: BufferLayout__namespace.struct([
            BufferLayout__namespace.u8('instruction'),
            BufferLayout__namespace.nu64('poolTokens'),
        ]),
    },
    ...
    IncreaseAdditionalValidatorStake: {
        ...
        layout: BufferLayout__namespace.struct([
            ...
            BufferLayout__namespace.nu64('lamports'),
            BufferLayout__namespace.nu64('transientStakeSeed'),
            BufferLayout__namespace.nu64('ephemeralStakeSeed'),
        ]),
    },
    ...
    Redelegate: {
        ...
        layout: BufferLayout__namespace.struct([
            ...
            BufferLayout__namespace.nu64('sourceTransientStakeSeed'),
            ...
            /// it can be anything
            BufferLayout__namespace.nu64('destinationTransientStakeSeed'),
        ]),
    },
});
/**
...
async function increaseValidatorStake(connection, stakePoolAddress, validatorVote, lamports, ephemeralStakeSeed) {
    ...
    if (ephemeralStakeSeed != undefined) {
        const ephemeralStake = await findEphemeralStakeProgramAddress(STAKE_POOL_PROGRAM_ID, stakePoolAddress, new BN__default["default"](ephemeralStakeSeed));
...
async function redelegate(props) {
    ...
    const sourceTransientStake = await findTransientStakeProgramAddress(STAKE_POOL_PROGRAM_ID, sourceVoteAccount, stakePoolAddress, new BN__default["default"](sourceTransientStakeSeed));
    ...
    const destinationTransientStake = await findTransientStakeProgramAddress(STAKE_POOL_PROGRAM_ID, destinationVoteAccount, stakePoolAddress, new BN__default["default"](destinationTransientStakeSeed));
    const ephemeralStake = await findEphemeralStakeProgramAddress(STAKE_POOL_PROGRAM_ID, stakePoolAddress, new BN__default["default"](ephemeralStakeSeed));
    const instructions = [];
...
async function initialize(props) {
    ...
    const manager = (_e = props.manager) !== null && _e !== void 0 ? _e : web3_js.Keypair.generate();
    const instructions = [];
